...
- **FR-15.** ~~Clipboard contents, remote screen contents, remote command output, and remote source code are not collected by the local RDP recorder. A paste is recorded only as a marker.~~ **DEPRECATED / УСТАРЕВШИЙ** — superseded by screen capture with OCR for applications that block keyboard/output capture.
- **FR-15.** ~~Содержимое буфера обмена, экран удаленной машины, вывод удаленных команд и исходный код на удаленной машине локальный RDP-рекордер не собирает. Вставка записывается только как маркер.~~ **УСТАРЕВШИЙ / DEPRECATED** — заменён сбором скриншотов с распознаванием текста для приложений, блокирующих захват клавиатуры и вывода.

## Operational Boundaries / Эксплуатационные ограничения

- **FR-16.** ~~RDP recording must be started only after RDP authentication and stopped before entering passwords, tokens, or other secrets, because the local collector cannot distinguish a remote password prompt from a terminal.~~ **DEPRECATED / УСТАРЕВШИЙ** — screenshot OCR replaces the need to stop before secrets; recognize the unavoidable capture of on-screen secrets and keep the existing command/field redaction.
- **FR-16.** ~~Запись RDP необходимо запускать только после аутентификации в RDP и останавливать до ввода паролей, токенов или других секретов, так как локальный коллектор не отличает удаленный парольный запрос от терминала.~~ **УСТАРЕВШИЙ / DEPRECATED** — OCR скриншотов делает остановку до ввода секретов ненужной; признаётся неизбежный захват секретов с экрана, применяется существующая редиакция команд и полей.
...
- **FR-27.** The local RDP recorder captures the window it is recording as an image and recognizes on-screen text for applications that block local keyboard and output capture (e.g. remote desktops). A `WindowScreenCapture` module grabs the selected window through GDI (`PrintWindow`), stores the image as PNG, and a `WindowOcr` module recognizes text over the local Windows OCR engine via the `winrt-Windows.Media.Ocr` package. Capture and recognition are triggered: (1) around every Enter in the recorded window — one `app.screen_text` event before the command and one after, paired by an incrementing numeric `command_id`; (2) when the foreground window changes away from the recorded window — `trigger` = `focus_change`; (3) while raw keyboard is active without a decoding low-level hook — periodic `trigger` = `keyboard_unavailable`. Screenshots are rate-limited to at most 2 frames per second. Entire window text is recognized in one pass; recognized text is saved next to the PNG in a UTF-8 `.txt` file with the same base name. When the new frame is pixel-identical to the previous one, no new PNG/`.txt` is written and the event reuses the stored file with `duplicate: true`. Each `app.screen_text` event includes window context plus `screen: {trigger, command_id, screenshot, width, height, text, lines[{x,y,w,h,text,words[{x,y,w,h,text}]}], duplicate, size_bytes}`. Receiving options: `--screenshot-dir`, `--ocr-language`, `--no-screenshots` (the collector and the `run_windows_collectors.py` launcher); screenshots default to the `screenshots/` directory next to the output JSONL.
- **FR-27.** Локальный RDP-рекордер делает скриншот записываемого окна и распознаёт текст на экране для приложений, которые блокируют локальный захват клавиатуры и вывода (например, удалённые рабочие столы). Модуль `WindowScreenCapture` снимает выбранное окно через GDI (`PrintWindow`), сохраняет изображение в PNG, а модуль `WindowOcr` распознаёт текст через локальный OCR-движок Windows пакетом `winrt-Windows.Media.Ocr`. Съёмка и распознавание запускаются: (1) вокруг каждого Enter в записываемом окне — одно событие `app.screen_text` до команды и одно после, связанные общим числовым `command_id`; (2) при смене активного окна с записываемого на другое — `trigger` = `focus_change`; (3) при активном raw keyboard без декодирующего low-level хука — периодически с `trigger` = `keyboard_unavailable`. Скриншоты ограничены не более 2 кадров в секунду. Текст окна распознаётся целиком за один проход; распознанный текст сохраняется рядом с PNG в UTF-8 `.txt`-файл с тем же базовым именем. Если новый кадр пиксельно идентичен предыдущему, новый PNG/`.txt` не пишется, и событие ссылается на уже сохранённый файл с `duplicate: true`. Каждое событие `app.screen_text` содержит контекст окна и поле `screen: {trigger, command_id, screenshot, width, height, text, lines[{x,y,w,h,text,words[{x,y,w,h,text}]}], duplicate, size_bytes}`. Опции приёма: `--screenshot-dir`, `--ocr-language`, `--no-screenshots` (в коллекторе и лаунчере `run_windows_collectors.py`); скриншоты по умолчанию сохраняются в каталог `screenshots/` рядом с выходным JSONL.
