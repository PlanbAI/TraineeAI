...
if [[ ! -f LinuxCollector.py ]]; then
  echo "ERROR: LinuxCollector.py not found in repository root" >&2
  exit 3
fi
...
"$PYTHON_BIN" LinuxCollector.py >"$OUTPUT_DIR/linux-collector.stdout.log" 2>"$OUTPUT_DIR/linux-collector.stderr.log" &
LINUX_COLLECTOR_PID=$!
